...
date: "`r Sys.Date()`"
author: "Universidade de São Paulo <\br> Texas A&M University <\br> Montana State University"
output:
...
Utilizar essas operações exige conhecimento de álgebra de matrizes, se quiser se aprofundar, o livro *Linear Models in Statistics, Rencher (2008)* possui um boa revisão. Você também pode explorar a sintaxe do R para essas operações neste [link](http://www.statmethods.net/advstats/matrix.html).
...
Às vezes pode ser mais informativo dar nomes às colunas e linhas da matriz. Para isso:
...
Mesmo se tratando de um *data frame*, podemos realizar operações com os vetores numéricos que o compõem.
...
Listas consistem em uma coleção de objetos, não necessariamente de mesma classe. Nelas podemos armazenar todos os outros objetos já vistos e recuperá-los pela indexação com `[[`. Vamos utilizar alguns objetos que já foram gerados como um exemplo:
...
Listas são muito úteis quando vamos utilizar/gerar diversos objetos dentro de um loop. 
